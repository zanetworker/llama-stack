        ...
        for message in st.session_state.messages:
            with st.chat_message(message["role"]):
                try:
                    ...
                    st.warning(f"Note: Failed to display message. Error: {str(e)}")
        ...
        if prompt := st.chat_input("What's your RAG use case?"):
            ...
            with st.chat_message("assistant"):
                if not extracted_params["use_case"]:
                    ...
                    # Add key parameters as separate elements
                    st.write("Chunk Size:", parameters.get('chunk_size_tokens', 'Not specified'))
                    st.write("Chunk Overlap:", parameters.get('overlap_tokens', 'Not specified'))
                    st.write("Embedding Model:", parameters.get('embedding_model_recommendation', 'Not specified'))
                    st.write("Top-K:", parameters.get('top_k', 'Not specified'))
                    st.write("Retrieval Enhancements:", parameters.get('retrieval_enhancements', 'Not specified'))
                    st.write("Generation Settings:", parameters.get('generation_settings', 'Not specified'))
                    st.write("Chunking Strategy:", parameters.get('chunking_strategy_recommendation', 'Not specified'))
                    
                    # Add explanation
                    st.write("Would you like me to explain any of these parameters in more detail or adjust them based on additional requirements?")
                    ...
                    # Construct a simplified response for chat history
                    chat_response = f"Based on your {extracted_params['use_case']} use case, I've recommended parameters for chunk size, overlap, embedding model, and more. Would you like me to explain any of these in more detail?"
            ...
            if run_button:
                if not uploaded_files:
                    ...
                    if result["success"]:
                        ...
                        # Use text area for the response to avoid Markdown rendering issues
